		...
		*/
		$('#nrp,#n6').keyup(function(event){
			...
			if( (barcode && $('#nrp').val().length == 8) || (!barcode && $('#n6').val().length == 3) )
			{
				...
				if(char_num == 13)
				{
					var url = "<?php echo site_url("mhs_absensi_upacara/action").'/'.$id;?>/"+data;
					$.get(url, function(ajaxResult) {
						...
						else if(result==0) // nrp tidak terdaftar
						{
							...
							if (answer)
							{
								var url = "<?php echo site_url("mhs_absensi_upacara/tambah").'/'.$id;?>/"+data;
								$.get(url, function(ajaxAddResult) {
									alert("Mahasiswa dengan NRP "+data+" berhasil ditambahkan!");
								});
							}
						}
						else if(result==1)//ga lulus
						{
							var msg=message.substr(64);
							message=message.substr(0,63);
							alert(message);
							$('#history').html(msg+$('#history').html());
							$('#n6').val("");
						}
						else if(result==4)
						{
							var msg=message.substr(55);
							message=message.substr(0,54);
							alert(message);
							$('#history').html(msg+$('#history').html());
							$('#n6').val("");
						}
				...
				{
					var url = "<?php echo site_url("mhs_absensi_upacara/nama");?>/"+data;
